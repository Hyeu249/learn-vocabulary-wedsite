...
form.addEventListener("keyup", (e) => {
    if (e.keyCode == 13) {
        if (storeVocabOut[form.value.trim()] == undefined) {
            // Khắc phục dấu cách
            if (form.value.trim() == "") { form.value.trim() = " " };
            // Gửi từ vựng Sai đi
            wrongWordFunction(form.value.trim());
            // Tắt hiển thị từ vựng đúng ở đầu bảng
            let showCurrentlyVocab = document.querySelector(".currentlyRight");
            // showCurrentlyVocab.innerHTML = "";
            showCurrentlyVocab.classList.add('red');
            showCurrentlyVocab.textContent = form.value.trim();
...
function displayVocab() {
    ...
    let RighWords = localStorage.getItem('RighWords');
    RighWords = JSON.parse(RighWords);
    if (RighWords) { var allItem = Object.values(RighWords) }
    ...
    timeCountHourglass.style.width = `${(550 / numVocab) * childTextVocab.length}px`
}
...
function cartVocab() {
    ...
    if (resultNumberRight) {
        ...
        if (numVocabRight.reduce((a, b) => a + b.wordR, 0) > 0) { cartNumbers.classList.add('cartVocab-color') };
...
function VocabWrong() {
    ...
    if (resultNumberWrong) {
        ...
        if (numVocabWrong.reduce((a, b) => a + b.wordW, 0) > 0) { cartNumberWrong.classList.add('VocabWrong-Color') };
    }

    if (numVocabWrong) { cartNumberWrong.textContent = numVocabWrong.reduce((a, b) => a + b.wordW, 0) }
}
VocabWrong();
...
function countVocab(num, YLS, RS, TIME) {
    ...
    setTimeout(() => { childStartHourglass.classList.remove('none') }, 60)
    ...
    if (num > 0) {
        ...
        let setTimme1_5 = setTimeout(function () {
            ...
            if (storageNumber) { var numVocabAll = Object.values(storageNumber) } else { numVocabAll = 0; }
            ...
            for (let i = forLength - 1; i >= 0; i--) {
                if (y < YelCountStart) {
                    ...
                    let currentVocab = ""
                    if (numVocabAll.length !== YelCountStart) currentVocab = numVocabAll[i - 1].name;
                    ...
                    if (numVocabAll[i - 1] === undefined) { showCurrentlyVocab.textContent = "" } else if (numVocabAll[i - 1] !== undefined) { showCurrentlyVocab.innerHTML = `${currentVocab}` };
                }
                y++;
            }

            localStorage.setItem('RighWords', JSON.stringify(storageNumber));
            if (numVocabAll.length <= YelCountStart) localStorage.removeItem("RighWords");
            cartVocab();
        }, TIMEAfter + 60);
        ...
        let setTimme2_5 = setTimeout(function () {
            ...
            for (let i = forLength - 1; i >= 0; i--) {
                if (y < RedCountStart) {
                    ...
                    // show từ vựng hiện tại
                    let currentVocab = "";
                    if (numVocabAll.length !== RedCountStart) currentVocab = numVocabAll[i - 1].name;
        ...
        let setTime3 = setTimeout(function () { location.reload(); }, TIMEAfter * 3 + 60);
...
function startTest() {
    function startTestEvent() {
        ...
        if (yellowCountStart.value == "") { yellowCountStart.value = 0; }
        if (redCountStart.value == "") { redCountStart.value = 0 }

        if (document.querySelector(".modeLowerAct")) { countVocab(1, yellowCountStart.value, redCountStart.value, 5000); };
...
function findParenthesis3(showCurrentVOcab) {
    let locationPThree = showCurrentVOcab.indexOf("(", showCurrentVOcab.indexOf("(") + 1);
